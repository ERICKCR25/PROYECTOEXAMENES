...
public class ExamenMySqlRepositorio implements ExamenRepositorio {
	...
	public int elimina(int idExamen) {
		String sql = "update Examen set claves='',numPreguntas=0 where idExamen=?";
		Object[] val = { idExamen };
		return jdbcTemplate.update(sql, val);
	}
	...
	public int actualiza(Examen obj) {
		String sql = "update Examen set nombre=?,descripcion=?,calves=?,numPreguntas=?,obj.valorAprobatorio=?, porAprobacion=?, valorTotal=? where idExamen=?";
